...
class AvalancheStyleWeb3Service {
  static final AvalancheStyleWeb3Service _instance = AvalancheStyleWeb3Service._internal();
  factory AvalancheStyleWeb3Service() => _instance;
  AvalancheStyleWeb3Service._internal();

  // Somnia Testnet configuration (adapted from Avalanche)
  ...
  void initialize([String network = ContractsConfig.defaultNetwork]) {
    ...
    print('🎯 AvalancheStyleWeb3Service: Initialized with Somnia Testnet contracts:');
  ...
  // Create event on blockchain using direct Web3Dart approach (like Avalanche)
  Future<String> createEvent({
    ...
    try {
      print('🎯 AvalancheStyleWeb3Service: Creating event on blockchain...');
      print('📝 Event Details:');
      ...
      // Prepare the transaction parameters (matching Avalanche approach)
      final parameters = [
      ...
      // Create the transaction with proper gas estimation (like Avalanche)
      final transaction = Transaction.callContract(
        contract: contract,
        function: createEventFunction,
        parameters: parameters,
        maxGas: 2000000, // Increased gas limit like Avalanche
      );
      ...
      print('✅ AvalancheStyleWeb3Service: Event creation transaction sent successfully');
      print('📝 Transaction Hash: $txHash');
      print('🔗 View on explorer: https://shannon-explorer.somnia.network/tx/$txHash');

      return txHash;
    } catch (e) {
      print('❌ AvalancheStyleWeb3Service: Error creating event: $e');
  ...
  // Wait for transaction confirmation (like Avalanche)
  Future<TransactionReceipt?> waitForTransactionConfirmation(String txHash) async {
    try {
      print('⏳ AvalancheStyleWeb3Service: Waiting for transaction confirmation...');
      print('📝 Transaction Hash: $txHash');
      ...
      while (receipt == null && attempts < maxAttempts) {
        ...
        try {
          ...
          if (receipt != null) {
            print('✅ AvalancheStyleWeb3Service: Transaction confirmed!');
          ...
          if (attempts % 10 == 0) { // Log every 10 seconds
            print('⏳ AvalancheStyleWeb3Service: Still waiting for confirmation... (${attempts + 1}s)');
          }
        }
        attempts++;
      }
      ...
      if (receipt == null) {
        print('⚠️ AvalancheStyleWeb3Service: Transaction confirmation timeout after ${maxAttempts} seconds');
      ...
      print('❌ AvalancheStyleWeb3Service: Error waiting for transaction confirmation: $e');
      rethrow;
    }
  }
