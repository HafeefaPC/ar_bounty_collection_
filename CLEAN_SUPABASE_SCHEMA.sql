-- FaceReflector AR Bounty Collection App - Clean Supabase Schema
...
-- Goodies table for rewards and prizes
CREATE TABLE public.goodies (
    ...
    CONSTRAINT valid_goodie_coordinates CHECK (latitude BETWEEN -90 AND 90 AND longitude BETWEEN -180 AND 180),
    CONSTRAINT valid_claim_radius CHECK (claim_radius > 0 AND claim_radius <= 1000)
);
...
-- Create geography column from lat/lng for boundaries
...
CREATE INDEX idx_boundaries_location ON public.boundaries USING GIST(location);
...
CREATE INDEX idx_goodies_claimed ON public.goodies(is_claimed);
...
-- Create admin user
INSERT INTO public.users (wallet_address, username, display_name, preferences)
VALUES (
    '0x84efBdc3146C76066591496A34e08b4e12fe8d2F', 
    'Admin User', 
    'Admin',
...
COMMENT ON SCHEMA public IS 'FaceReflector AR Bounty Collection App - Clean Database Schema';
COMMENT ON TABLE public.users IS 'User accounts with wallet-based authentication';
COMMENT ON TABLE public.events IS 'AR events with location and blockchain integration';
COMMENT ON TABLE public.boundaries IS 'NFT boundaries that users can claim at events';
COMMENT ON TABLE public.goodies IS 'Rewards and prizes available at events';
COMMENT ON TABLE public.event_participants IS 'Users participating in events';
COMMENT ON TABLE public.user_proximity_logs IS 'Location tracking for analytics';
COMMENT ON TABLE public.nft_claims IS 'Detailed tracking of NFT boundary claims';
